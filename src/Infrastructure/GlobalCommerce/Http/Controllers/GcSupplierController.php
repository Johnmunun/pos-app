...
class GcSupplierController
{
    ...
    public function importPreview(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::error('GcSupplier import preview: parse error', ['error' => $e->getMessage()]);
        ...
        foreach ($dataRows as $index => $row) {
            ...
            if ($name !== '') {
                $key = mb_strtolower($name) . '|' . mb_strtolower($rowAssoc['telephone'] ?? '');
            ...
            if (!empty($lineErrors)) {
                ...
                $valid++;
                if ($name !== '') {
                    $seen[mb_strtolower($name) . '|' . mb_strtolower($rowAssoc['telephone'] ?? '')] = true;
                }
                if ($email !== '') {
