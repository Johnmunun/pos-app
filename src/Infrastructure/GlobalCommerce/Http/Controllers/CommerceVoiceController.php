...
class CommerceVoiceController
{
    ...
    /**
     * POST /commerce/api/voice/transcribe
    ...
    public function transcribe(Request $request): JsonResponse
    {
        ...
        if (!$this->checkVoiceQuota($shopId)) {
            $max = config('commerce_assistant.voice_max_requests_per_day', self::DEFAULT_MAX_VOICE_REQUESTS_PER_DAY);
    ...
    /**
     * GET /commerce/api/voice/settings
     */
    ...
    /**
     * PUT /commerce/api/voice/settings
     */
    public function updateSettings(Request $request): JsonResponse
    {
        ...
        $setting = CommerceAssistantSetting::firstOrNew(['shop_id' => $shopId], CommerceAssistantSetting::defaults());
    ...
    {
        $s = CommerceAssistantSetting::where('shop_id', $shopId)->first();
        $defaults = CommerceAssistantSetting::defaults();
        if (!$s) {
            return $defaults;
        }
        return [
    ...
    {
        $max = config('commerce_assistant.voice_max_requests_per_day', self::DEFAULT_MAX_VOICE_REQUESTS_PER_DAY);
        $key = 'voice_requests:commerce:' . $shopId . ':' . now()->format('Y-m-d');
