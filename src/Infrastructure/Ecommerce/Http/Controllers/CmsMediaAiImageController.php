...
class CmsMediaAiImageController
{
    ...
    public function generate(Request $request): JsonResponse
    {
        ...
        $this->featureLimitService->assertFeatureEnabled($tenantId, 'ai.media.image.generate');
        $this->featureLimitService->assertCanUseMonthlyFeature(
            $tenantId,
            'ai.media.image.generate',
            'la génération d’image IA Media'
        );
        ...
        if ($isAsync && Schema::hasTable('ai_image_generation_requests')) {
            $row = AiImageGenerationRequest::query()->create([
                'tenant_id' => $tenantId,
                'feature_code' => 'ai.media.image.generate',
                'context' => 'media',
                'status' => 'pending',
                'count' => $count,
                'title' => $title,
        ...
        $prompt = "Génère une image de média CMS e-commerce propre et esthétique.\n"
            ."Titre: {$title}\n"
        ...
        $this->featureLimitService->recordFeatureUsage($tenantId, 'ai.media.image.generate', count($images));
    ...
    public function status(Request $request, string $id): JsonResponse
    {
        ...
        $row = AiImageGenerationRequest::query()
            ...
            ->where('feature_code', 'ai.media.image.generate')
            ->first();
        ...
        if ($row->status === 'completed') {
            ...
            if (!empty($images)) {
                ...
                $response['file_name'] = $images[0]['file_name'] ?? 'cms-ai-image.png';
            }
        }
        ...
        foreach ($attempts as $attempt) {
            try {
                ...
                if (!$response->successful()) {
                    ...
                    Log::warning('CMS Media AI image generation API error', [
                        'model' => $attempt['model'],
                ...
                Log::warning('CMS Media AI image generation HTTP error', ['message' => $e->getMessage()]);
            }
        }
