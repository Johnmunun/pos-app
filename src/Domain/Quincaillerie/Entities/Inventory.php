 ...
 *
 * Représente un inventaire physique du stock pour le module Quincaillerie.
...
class Inventory
{
    ...
    {
        return 'INV-HW-' . $date->format('Ymd') . '-' . strtoupper(substr(Uuid::uuid4()->toString(), 0, 6));
    }
     ...
     */
    public function getTotalPositiveDifference(): float
    {
        $total = 0.0;
     ...
     */
    public function getTotalNegativeDifference(): float
    {
        $total = 0.0;
        foreach ($this->items as $item) {
            $diff = $item->getDifference();
