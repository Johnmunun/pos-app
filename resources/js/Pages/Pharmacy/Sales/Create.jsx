...
export default function POSCreate({ products = [], categories = [], customers = [], canUseWholesale = false, cashRegisters = [], currency: pageCurrency, currencies: pageCurrencies = [], exchangeRates: pageExchangeRates = {}, routePrefix = 'pharmacy', loyaltySettings = { enabled: false }, loyaltyModule }) {
    ...
    const resolvedLoyaltyModule = loyaltyModule || (routePrefix === 'hardware' ? 'hardware' : 'pharmacy');
    ...
    return (
        <AppLayout fullWidth>
            ...
            <div
                className={`pos-sale-create pos-sale-create--pharmacy pos-sale-create__shell h-[calc(100dvh-4rem)] max-h-[calc(100dvh-4rem)] flex flex-col lg:flex-row relative overflow-hidden${cart.length > 0 ? ' pos-sale-create--has-cart' : ''}`}
            >
                ...
                {/* Left Panel - Products */}
                <div className="pos-sale-create__products w-full md:flex-1 flex flex-col bg-gray-50 dark:bg-slate-950 min-w-0 overflow-hidden">
                    {/* Header with Search and View Toggle */}
                    <div className="pos-sale-create__header p-3 sm:p-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 shrink-0">
                        <div className="flex flex-col md:flex-row md:items-center gap-3 sm:gap-4 md:flex-wrap">
                            ...
                            <div className="pos-sale-create__sale-mode flex items-center bg-amber-50 dark:bg-amber-900/20 rounded-lg p-1 border border-amber-200 dark:border-amber-800" title={saleMode === 'wholesale' && !canUseWholesale ? 'Droits vente en gros requis' : ''}>
                                <button
                                    type="button"
                            ...
                            <div className="pos-sale-create__search-row w-full md:flex-1 md:min-w-[200px] min-w-0 relative flex items-center gap-2">
                                {availableCurrencies.length >= 1 && (
                                    <select
                                        value={currency}
                                        onChange={(e) => setSelectedCurrency(e.target.value)}
                    ...
                    {/* Categories */}
                    <div className="pos-sale-create__categories p-3 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 overflow-x-auto shrink-0 touch-pan-x">
                    ...
                    {recentProductIds.length > 0 && (
                        <div className="pos-sale-create__recent px-4 py-2 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700">
                    ...
                    {/* Products Grid/List */}
                    <div className="pos-sale-create__scroll flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-4 min-h-0">
                        {viewMode === 'thumbnails' ? (
                            <div className="pos-sale-create__grid grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-1.5 sm:gap-3 md:gap-4">
                ...
                {/* Right Panel - Cart */}
                <div className="pos-sale-create__cart w-full lg:w-80 lg:min-w-[18rem] lg:max-w-sm xl:w-96 xl:min-w-[22rem] xl:max-w-md bg-white dark:bg-slate-900 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-slate-700 flex flex-col flex-shrink-0 min-h-0 overflow-hidden lg:h-full lg:max-h-none">
                    {/* Cart Header */}
            ...
            >
                        <div className="space-y-4">
                            ...
                            <LoyaltyPosPanel
                                ...
                                loyaltyModule={resolvedLoyaltyModule}
