...
export default function POSCreate({ products = [], categories = [], customers = [], canUseWholesale = false, cashRegisters = [], currency: pageCurrency, currencies: pageCurrencies = [], exchangeRates: pageExchangeRates = {}, routePrefix = 'pharmacy', loyaltySettings = { enabled: false }, loyaltyModule }) {
    ...
    return (
        <AppLayout fullWidth>
            ...
            <div
                className={`pos-sale-create pos-sale-create--pharmacy pos-sale-create__shell h-[calc(100dvh-4rem)] max-h-[calc(100dvh-4rem)] flex flex-col md:flex-row relative overflow-hidden${cart.length > 0 ? ' pos-sale-create--has-cart' : ''}`}
            >
                ...
                {/* Right Panel - Cart */}
                <div className="pos-sale-create__cart w-full md:w-80 md:min-w-[18rem] md:max-w-sm lg:w-96 lg:min-w-[22rem] lg:max-w-md bg-white dark:bg-slate-900 border-t md:border-t-0 md:border-l border-gray-200 dark:border-slate-700 flex flex-col flex-shrink-0 min-h-0 overflow-hidden max-md:max-h-[min(46dvh,440px)] md:h-full md:max-h-none">
                    {/* Cart Header */}
