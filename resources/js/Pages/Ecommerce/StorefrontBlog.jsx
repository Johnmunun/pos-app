...
function BlogContent({ shop, articles = [], cmsPages = [], whatsapp = {}, pageSeo = null }) {
    const { shop: sharedShop } = usePage().props;
                        ...
                        {articles.length === 0 ? (
                            ...
                            <div className="grid gap-6 sm:gap-8 md:grid-cols-2">
                                {articles.map((article) => (
                                    <Link
                                        key={article.id}
                                        href={links.blogShow(article.slug)}
...
export default function StorefrontBlog({ shop, articles = [], cmsPages = [], whatsapp = {}, pageSeo = null }) {
    const currency = shop?.currency || 'CDF';

    return (
        <CartProvider currency={currency} storageKey={`ecommerce_cart_${shop?.id ?? 'default'}`}>
            <BlogContent shop={shop} articles={articles} cmsPages={cmsPages} whatsapp={whatsapp} pageSeo={pageSeo} />
        </CartProvider>
    );
}
