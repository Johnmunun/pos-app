...
function QuickAddCustomerModal({ onClose, onCreated, routePrefix = 'commerce' }) {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50" onClick={onClose}>
            <div
                className="bg-white dark:bg-slate-900 rounded-xl w-full max-w-sm mx-4 p-6 shadow-xl"
...
export default function CommerceSalesCreate({
    products = [],
    categories = [],
    customers = [],
    ...
    function generateOrderNumber() {
        return 'C01-' + String(Math.floor(Math.random() * 900000000) + 100000000).padStart(10, '0');
    }
    ...
    const getMinAllowedForProduct = (product) => {
        ...
        const retailMin = product.min_sale_price_amount ?? null;
        const wholesaleMin = product.min_wholesale_price_amount ?? null;
    ...
    const getProductPrice = (product) => {
        const wholesale =
            product?.wholesale_price_amount ?? product?.wholesale_price ?? null;
        if (saleMode === 'wholesale' && canUseWholesale && wholesale != null) {
            return wholesale;
        }
        return (
    ...
    const addToCart = (product) => {
        ...
        const estDivisible = Boolean(product.est_divisible ?? false);
            ...
            setCart((prev) =>
                prev.map((item) =>
                    item.product_id === product.id
                        ? {
                              ...item,
                              quantity: estDivisible
                                  ? item.quantity + 1
                                  : Math.min(item.quantity + 1, Math.floor(stock)),
                          }
                        : item,
                ),
            );
    ...
    const subtotal = useMemo(
        () =>
            cart.reduce((sum, item) => {
                ...
                return (
                    sum + convertToSelected(lineTotal, item.price_currency ?? defaultCurrency)
                );
            }, 0),
        ...
        const onKey = (e) => {
            ...
            if (e.key === '+' || e.key === '=') {
                ...
                if (val !== null) {
                    ...
                    setCart((prev) =>
                        prev.map((i) =>
                            i.product_id === item.product_id ? { ...i, discount_percent: pct } : i,
                        ),
                    );
                }
                e.preventDefault();
    ...
    const handleFinalize = async () => {
        ...
        try {
            ...
            if (shop?.receipt_auto_print) {
                window.open(
                    route(`${routePrefix}.sales.receipt`, saleId),
                    '_blank',
                    'noopener,noreferrer',
                );
            }
                                ...
                                {recentProductIds.map((id) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span className="text-sm font-medium text-gray-900 dark:text-white max-w-full sm:max-w-[100px] truncate">
                                                {product.name}
                                            </span>
                                            <span className="text-xs text-amber-600 dark:text-amber-400 font-semibold">
                                                {fmt(
                                                    convertToSelected(
                                                        ...
                                                        product.price_currency ?? defaultCurrency,
                                                    ),
                                                )}
                                            </span>
                                        </button>
                                ...
                                {filteredProducts.map((product) => (
                                    ...
                                    >
                                        ...
                                        {product.stock > 0 && product.stock < LOW_STOCK_THRESHOLD && (
                                            <Badge
                                                variant="outline"
                                                className="mt-1 text-xs text-orange-600 dark:text-orange-400 border-orange-300 dark:border-orange-600"
                                            >
                                                Rupture bientôt
                                            </Badge>
                                        )}
                                            ...
                                            {canUseWholesale && product.wholesale_price_amount != null && (
                                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                                    {saleMode === 'wholesale'
                                                        ...
                                                        : `Gros: ${fmt(
                                                              convertToSelected(
                                                                  product.wholesale_price_amount,
                                                                  product.price_currency ??
                                                                      defaultCurrency,
                                                              ),
                                                          )}`}
                ...
                <div className="w-full lg:w-96 lg:min-w-[22rem] lg:max-w-md bg-white dark:bg-slate-900 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-slate-700 flex flex-col flex-shrink-0 lg:flex-shrink-0">
                    ...
                    <div className="p-4 border-b border-gray-200 dark:border-slate-700">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            >
                                {saleMode === 'wholesale' ? (
                                    <>
                                        <Package className="h-3 w-3 mr-1 inline" /> Vente en gros
                                    </>
                                ) : (
                                    <>
                                        <Store className="h-3 w-3 mr-1 inline" /> Vente au détail
                                    </>
                                )}
                            </Badge>
                        </div>
                    ...
                    {cart.length > 0 && (
                        <p className="px-4 py-1 text-xs text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-slate-800">
                            ...
                            <kbd className="px-1 rounded bg-gray-200 dark:bg-gray-700 font-mono ml-0.5">
                                -
                            </kbd>{' '}
                            quantité ·{' '}
                            <kbd className="px-1 rounded bg-gray-200 dark:bg-gray-700 font-mono">
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-2">
                                                    ...
                                                    onClick={() =>
                                                        updateQuantity(
                                                            ...
                                                            item.est_divisible !== false
                                                                ? Math.round((item.quantity - 0.5) * 100) / 100
                                                                : item.quantity - 1,
                                                        )
                                                    }
                                                ...
                                                <input
                                                    type="number"
                                                    step={item.est_divisible !== false ? 0.01 : 1}
                                                    ...
                                                    title={
                                                        item.est_divisible !== false
                                                            ? 'Quantité (ex. 0.5, 0.3 — prix et stock proportionnels)'
                                                            : 'Quantité entière uniquement'
                                                    }
                                                />
                                                <button
                                                    type="button"
                                            ...
                                            <p className="font-semibold text-gray-900 dark:text-white">
                                                {fmt(
                                                    convertToSelected(
                                                        ...
                                                        item.price_currency ?? defaultCurrency,
                                                    ),
                                                )}
                                            </p>
                    ...
                    <div className="border-t border-gray-200 dark:border-slate-700 p-4 space-y-3">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between text-gray-600 dark:text-gray-400">
                                <span>
                                    Sous-total{' '}
                                    <span className="text-xs">({cart.length} articles)</span>
                                </span>
                            ...
                            <div className="rounded-xl p-4 min-h-[72px] flex flex-col justify-center">
                                {change > 0 && (
                                    <div className="text-center">
                                        ...
                                        <p className="text-3xl font-bold text-green-600 dark:text-green-400 mt-0.5">
                                            {fmt(change)}
                                        </p>
                                    </div>
                                )}
                                {balance > 0 && paid > 0 && (
                                    <div className="text-center">
                                        ...
                                        <p className="text-3xl font-bold text-red-600 dark:text-red-400 mt-0.5">
                                            {fmt(balance)}
                                        </p>
                                    </div>
                                )}
