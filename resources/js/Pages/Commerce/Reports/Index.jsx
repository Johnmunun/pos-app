...
export default function ReportsIndex({ report, filters, routePrefix = 'commerce' }) {
    const { shop } = usePage().props;
    // Priorité : devise du rapport (shop utilisé) > devise partagée > CDF
    const currency = report?.currency || shop?.currency || 'CDF';
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <div className="flex items-center gap-2">
                            <Button type="button" variant="outline" size="sm" className="h-9" asChild>
                                <a href={route(`${routePrefix}.exports.reports.pdf`, { from: dateFrom, to: dateTo })} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2">
                                    <FileDown className="h-4 w-4 shrink-0" />
                                    Exporter PDF
                                </a>
                            </Button>
                            <Button type="button" variant="outline" size="sm" className="h-9" asChild>
                                <a href={route(`${routePrefix}.exports.reports.excel`, { from: dateFrom, to: dateTo })} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2">
                                    ...
                                    <thead>
                                        <tr className="bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
                                            ...
                                            <th className="text-right py-3 px-4 font-semibold text-gray-700 dark:text-gray-200">
                                                <button type="button" onClick={() => toggleSort('qty_sold')} className="inline-flex items-center gap-1 hover:text-gray-900 dark:hover:text-white">Qté vendue <ArrowUpDown className="h-3.5 w-3.5" /></button>
                                            </th>
                                            <th className="text-right py-3 px-4 font-semibold text-gray-700 dark:text-gray-200">
                                                <button type="button" onClick={() => toggleSort('revenue')} className="inline-flex items-center gap-1 hover:text-gray-900 dark:hover:text-white">Chiffre d&apos;affaires <ArrowUpDown className="h-3.5 w-3.5" /></button>
                                            </th>
                                            <th className="text-right py-3 px-4 font-semibold text-gray-700 dark:text-gray-200">Coût</th>
                                            <th className="text-right py-3 px-4 font-semibold text-gray-700 dark:text-gray-200">
                                                <button type="button" onClick={() => toggleSort('benefit')} className="inline-flex items-center gap-1 hover:text-gray-900 dark:hover:text-white">Bénéfice <ArrowUpDown className="h-3.5 w-3.5" /></button>
                                            </th>
                                            <th className="text-right py-3 px-4 font-semibold text-gray-700 dark:text-gray-200">
                                                <button type="button" onClick={() => toggleSort('margin_percent')} className="inline-flex items-center gap-1 hover:text-gray-900 dark:hover:text-white">Marge % <ArrowUpDown className="h-3.5 w-3.5" /></button>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {sortedProducts.map((row) => (
                                            <tr key={row.product_id} className="border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800/50">
                                                ...
                                                <td className="py-3 px-4 text-right">
                                                    {row.benefit != null ? (
                                                        <span className={row.benefit >= 0 ? 'text-green-600 dark:text-green-400 font-medium' : 'text-red-600 dark:text-red-400 font-medium'}>{fmt(row.benefit)}</span>
                                                    ) : (
                                                ...
                                                <td className="py-3 px-4 text-right">
                                                    {row.margin_percent != null ? (
                                                        <span className={row.margin_percent >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>{row.margin_percent} %</span>
                                                    ) : (
                                                        <span className="text-gray-400 dark:text-gray-500">—</span>
                                                    )}
                                                </td>
