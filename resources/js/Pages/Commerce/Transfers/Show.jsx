...
export default function CommerceTransferShow({ transfer, products }) {
    ...
    const canEdit = hasPermission('module.commerce') && transfer.status === 'draft';
    const canValidate = hasPermission('module.commerce') && transfer.status === 'draft';
    const canCancel = hasPermission('module.commerce') && transfer.status === 'draft';
    ...
    const handleAddItem = async (product) => {
        setLoading(true);
        try {
            const response = await axios.post(route('commerce.transfers.items.add', transfer.id), {
    ...
    const handleUpdateQuantity = async (itemId) => {
        ...
        if (qty < 0.0001) {
            toast.error('La quantité doit être au moins 0.0001');
            return;
        }
        ...
        try {
            const response = await axios.put(
                route('commerce.transfers.items.update', { id: transfer.id, itemId }),
                { quantity: qty }
            );
    ...
    const handleRemoveItem = async (itemId) => {
        ...
        try {
            const response = await axios.delete(
                route('commerce.transfers.items.remove', { id: transfer.id, itemId })
            );
    ...
    const handleValidate = async () => {
        ...
        try {
            const response = await axios.post(route('commerce.transfers.validate', transfer.id));
    ...
    const handleCancel = async () => {
        ...
        try {
            const response = await axios.post(route('commerce.transfers.cancel', transfer.id));
                                ...
                                <div className="relative mb-6">
                                    <div className="flex gap-3">
                                        ...
                                        <Input
                                            ...
                                            min="0.0001"
                                            value={addQuantity}
                                            onChange={(e) => setAddQuantity(Math.max(0.0001, parseFloat(e.target.value) || 0.0001))}
                                            className="w-24"
                                            placeholder="Qté"
                                        />
                                    </div>
                                        ...
                                        <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                                            {transfer.items.map((item, index) => (
                                                <tr key={item.id}>
                                                    ...
                                                    <td className="py-3 px-4 font-mono text-sm">{item.product_sku || '—'}</td>
                                                        ...
                                                        {editingItem === item.id ? (
                                                            <div className="flex items-center justify-center gap-2">
                                                                <Input
                                                                    ...
                                                                    min="0.0001"
                                                                    value={editQuantity}
                                                                    onChange={(e) => setEditQuantity(Math.max(0.0001, parseFloat(e.target.value) || 0.0001))}
