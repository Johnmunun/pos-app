...
export default function CommerceTransferCreate({ depots = [], products = [], currentShopId }) {
    ...
    const updateQuantity = (productId, quantity) => {
        const qty = Math.max(0.0001, parseFloat(quantity) || 0.0001);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(route('commerce.transfers.store'), {
            ...
            if (response.data.success) {
                ...
                router.visit(route('commerce.transfers.show', response.data.transfer_id));
            } else {
