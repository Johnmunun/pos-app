...
export default function InventoryShow({ inventory, items, stats, availableProducts = [], categories = [], permissions }) {
    ...
    const filteredItems = useMemo(() => {
        return items.filter(item => {
            ...
            if (showOnlyDifferences && Math.abs(item.difference) < 0.01) {
                return false;
            }
            return true;
        });
    ...
    const realTimeStats = useMemo(() => {
        ...
        items.forEach(item => {
            ...
            if (counted !== '' && counted !== null && counted !== undefined) {
                countedItems++;
                const countedFloat = parseFloat(counted) || 0;
                const diff = countedFloat - item.system_quantity;
                if (diff > 0.01) {
                    totalPositive += diff;
                    itemsWithDiff++;
                } else if (diff < -0.01) {
        ...
        return {
            ...
            total_positive: totalPositive.toFixed(2),
            total_negative: totalNegative.toFixed(2),
        };
    }, [items, counts]);
        ...
        try {
            const countsToSave = Object.entries(counts)
                ...
                .map(([productId, countedQuantity]) => ({
                    ...
                    counted_quantity: parseFloat(countedQuantity) || 0,
                }));
            ...
            await axios.post(route('hardware.inventories.counts', inventory.id), {
                counts: countsToSave,
            });
    ...
    const handleStart = () => {
        router.post(route('hardware.inventories.start', inventory.id));
    };

    const handleValidateClick = () => setShowValidateModal(true);
    const handleValidateConfirm = () => {
        setShowValidateModal(false);
        router.post(route('hardware.inventories.validate', inventory.id));
    };

    const handleCancelClick = () => setShowCancelModal(true);
    const handleCancelConfirm = () => {
        setShowCancelModal(false);
        router.post(route('hardware.inventories.cancel', inventory.id));
    };
        ...
        <AppLayout
            header={
                <div className="flex items-center gap-4">
                    <Link
                        href={route('hardware.inventories.index')}
                            ...
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400">
                                        {inventory.depot_name && (
                                            <span className="flex items-center gap-1">
                                                <Package className="h-4 w-4 text-amber-500" />
                                                Dépôt : {inventory.depot_name}
                                            </span>
                                        )}
                                        <span className="flex items-center gap-1">
                                                ...
                                                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
                                                    ...
                                                    <td className="px-4 py-3 whitespace-nowrap text-center">
                                                        {canEdit ? (
                                                            ...
                                                            <span className="text-sm font-medium text-gray-900 dark:text-white">
                                                                {item.counted_quantity !== null ? item.counted_quantity.toFixed(2) : '—'}
                                                            </span>
                                                        )}
                                                    </td>
                                                    <td className="px-4 py-3 whitespace-nowrap text-center">
