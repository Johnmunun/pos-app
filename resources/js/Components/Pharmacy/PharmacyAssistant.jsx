...
function buildWelcomeMessage(userName) {
    ...
    return {
        role: 'assistant',
        id: 'welcome',
        content: `${greet} Je suis votre assistant Pharmacie. Je peux vous renseigner sur les ventes, le stock, les péremptions, la navigation dans l’application et la recherche de produits. Posez votre question ou utilisez le micro.`,
    };
}
...
function NavigationAction({ navigation }) {
    return (
        <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-700 transition shadow-sm"
        >
...
export default function PharmacyAssistant({ bottomOffset = null }) {
    ...
    const hasPharmacy =
        ...
        permissions.includes('module.pharmacy') ||
    ...
    useEffect(() => {
        if (open && hasPharmacy) {
            axios
                .get(route('pharmacy.api.voice.settings'))
    ...
    const handleSend = async (textOverride = null) => {
        ...
        try {
            const { data } = await axios.post(route('pharmacy.assistant.ask'), {
                message: text,
                history,
            });
            if (data.navigation?.type === 'navigation' && data.navigation.route) {
                ...
                if (voiceSettings.voice_enabled && answer && !data.navigation) {
                    ...
                    try {
                        ...
                        setMessages((prev) => [
                            ...prev,
                            { role: 'assistant', content: `Synthèse vocale indisponible : ${msg}`, id: `ve-${Date.now()}` },
                        ]);
                        setSpeaking(false);
                        setVoiceStatus(null);
                    }
    ...
    const saveVoiceSettings = () => {
        axios
            .put(route('pharmacy.api.voice.settings.update'), voiceSettings)
            .then(({ data }) => {
        ...
        <>
            {!open && (
                <button
                    ...
                    className="fixed right-4 sm:right-6 z-40 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-emerald-600 text-white shadow-lg shadow-emerald-600/30 transition hover:scale-105 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2"
                    style={!bottomOffset ? { bottom: '96px' } : {}}
                    title="Assistant Pharmacie"
                    aria-label="Ouvrir l'assistant pharmacie"
                >
            ...
            {open && (
                <div
                    ...
                    role="dialog"
                    aria-label="Assistant Pharmacie"
                >
                    <div className="flex items-center justify-between gap-2 px-4 py-3 text-white shrink-0" style={HEADER_GRADIENT}>
                        <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                                <p className="text-sm font-semibold truncate">Assistant Pharmacie</p>
                                <p className="text-[11px] text-emerald-50/90 flex items-center gap-1">
                                    <Sparkles className="h-3 w-3" />
                                    Métier · données boutique · IA
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 min-h-0 bg-slate-50/50 dark:bg-slate-900/30">
                        ...
                        {speaking && (
                            <div className="flex justify-start">
                                <div className="inline-flex items-center gap-2 rounded-2xl rounded-bl-md bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 px-3 py-2 text-sm text-emerald-800 dark:text-emerald-200">
                        ...
                        <div className="px-3 pb-2 flex flex-wrap gap-1.5 bg-slate-50/50 dark:bg-slate-900/30">
                            {SUGGESTIONS.map((label) => (
                                <button
                                    ...
                                    className="rounded-full border border-emerald-200 dark:border-emerald-800 bg-white dark:bg-slate-900 px-2.5 py-1 text-[11px] font-medium text-emerald-800 dark:text-emerald-200 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 transition"
                                >
                                    {label}
                                </button>
                            ))}
                        ...
                        <div className="p-2 flex gap-2 items-end">
                            {recording ? (
                                ...
                                <button
                                    ...
                                    disabled={busy}
                                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:border-emerald-400 hover:text-emerald-700 disabled:opacity-50 transition"
                            ...
                            <textarea
                                ...
                                rows={1}
                                placeholder="Question sur ventes, stock, péremptions…"
                                className="flex-1 max-h-24 min-h-[40px] resize-none rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 disabled:opacity-50"
                                disabled={busy}
                            />
                            <button
                                type="button"
                                ...
                                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition"
                                aria-label="Envoyer"
                            >
