...
import { MessageCircle, X, Send, Circle, Image as ImageIcon } from 'lucide-react';
...
export default function SupportChatWidget({ liftForMobileBottomNav = false }) {
    ...
    const canUse = Boolean(user) && (permissions.includes('*')
    ...
    const ensureConversation = async () => {
        const res = await window.axios.post(route('support.chat.conversation'));
    ...
    const connectStream = (id, afterId) => {
        try {
            ...
            es.onerror = () => {
                // EventSource auto-reconnects. We keep it quiet.
            };
        } catch {
        }
    };
    ...
    const send = async () => {
        ...
        try {
            ...
            await window.axios.post(route('support.chat.send', conversationId), fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            setAttachment(null);
            if (fileRef.current) fileRef.current.value = '';
            setUnreadCount(0);
            // Message will arrive via stream, but in case of delay, we can optimistically add:
