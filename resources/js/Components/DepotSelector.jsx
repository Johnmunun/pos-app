...
export default function DepotSelector({ depots = [], currentDepot }) {
    ...
    return (
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            <Warehouse className="h-4 w-4 text-gray-500 dark:text-gray-400 shrink-0 hidden sm:block" />
            <select
                value={value}
                onChange={(e) => handleSwitch(e.target.value)}
                className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-xs sm:text-sm py-1.5 px-2 sm:px-3 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 dark:focus:ring-amber-400 min-w-[100px] sm:min-w-[140px] max-w-[140px] sm:max-w-none truncate"
            >
