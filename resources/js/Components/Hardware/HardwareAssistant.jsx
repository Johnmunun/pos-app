...
function buildWelcomeMessage(userName) {
    ...
    return {
        role: 'assistant',
        id: 'welcome',
        content: `${greet} Je suis votre assistant Quincaillerie. Je peux vous renseigner sur le stock, les ventes du jour, la recherche d'articles et la navigation dans le module Hardware. Posez votre question ou utilisez le micro.`,
    };
}
...
function NavigationAction({ navigation }) {
    return (
        <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-amber-600 px-3 py-2 text-sm font-medium text-white hover:bg-amber-700 transition shadow-sm"
        >
...
export default function HardwareAssistant({ bottomOffset = null }) {
    ...
    useEffect(() => {
        if (open && hasHardwareVoice) {
            axios
                .get(route('hardware.api.voice.settings'))
                .then(({ data }) => setVoiceSettings((prev) => ({ ...prev, ...data })))
                .catch(() => {});
        }
    }, [open, hasHardwareVoice]);
    ...
    const handleSend = async (textOverride = null) => {
        ...
        try {
            const { data } = await axios.post(route('hardware.assistant.ask'), {
                message: text,
                history,
            });
    ...
    const saveVoiceSettings = () => {
        axios
            .put(route('hardware.api.voice.settings.update'), voiceSettings)
            .then(({ data }) => {
        ...
        <>
            {!open && (
                <button
                    ...
                    className="fixed right-4 sm:right-6 z-40 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-amber-600 text-white shadow-lg shadow-amber-600/30 transition hover:scale-105 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2"
            ...
            {open && (
                <div
                    ...
                    role="dialog"
                    aria-label="Assistant Quincaillerie"
                >
                    <div className="flex items-center justify-between gap-2 px-4 py-3 text-white shrink-0" style={HEADER_GRADIENT}>
                        <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                                <p className="text-sm font-semibold truncate">Assistant Quincaillerie</p>
                                <p className="text-[11px] text-amber-50/90 flex items-center gap-1">
                                    <Sparkles className="h-3 w-3" />
                                    Stock · ventes · navigation · IA
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 min-h-0 bg-slate-50/50 dark:bg-slate-900/30">
                        ...
                        {speaking && (
                            <div className="flex justify-start">
                                <div className="inline-flex items-center gap-2 rounded-2xl rounded-bl-md bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 px-3 py-2 text-sm text-amber-800 dark:text-amber-200">
                        ...
                        <div className="px-3 pb-2 flex flex-wrap gap-1.5 bg-slate-50/50 dark:bg-slate-900/30">
                            {SUGGESTIONS.map((label) => (
                                <button
                                    ...
                                    className="rounded-full border border-amber-200 dark:border-amber-800 bg-white dark:bg-slate-900 px-2.5 py-1 text-[11px] font-medium text-amber-800 dark:text-amber-200 hover:bg-amber-50 dark:hover:bg-amber-950/40 transition"
                                >
                                    {label}
                                </button>
                            ))}
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shrink-0">
                        <div className="p-2 flex gap-2 items-end">
                            {hasHardwareVoice &&
                                (recording ? (
                                    ...
                                    <button
                                        ...
                                        disabled={busy}
                                        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:border-amber-400 hover:text-amber-700 disabled:opacity-50 transition"
                            ...
                            <textarea
                                ...
                                rows={1}
                                placeholder="Question sur stock, ventes, articles…"
                                className="flex-1 max-h-24 min-h-[40px] resize-none rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 disabled:opacity-50"
                                disabled={busy}
                            />
                            <button
                                type="button"
                                ...
                                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-600 text-white hover:bg-amber-700 disabled:opacity-50 transition"
                                aria-label="Envoyer"
                            >
                        ...
                        <div className="px-3 pb-2 flex items-center justify-between gap-2">
                            <p className="text-[10px] text-slate-400 leading-snug">
                                Données quincaillerie · vérifiez les chiffres critiques.
                            </p>
                            <button
                                type="button"
                                onClick={clearChat}
            ...
            {showVoiceSettings && hasHardwareVoice && (
                <div
                    ...
                    >
                        ...
                        <div className="flex gap-2 pt-1">
                            <button
                                ...
                                className="flex-1 rounded-xl bg-amber-600 px-3 py-2 text-sm font-medium text-white hover:bg-amber-700"
                            >
                                Enregistrer
                            </button>
                            <button
