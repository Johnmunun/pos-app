...
<head>
    ...
    <title>Reçu #{{ $receipt_number ?? $sale['id'] }}</title>
    <style>
...
<body>
    <div class="receipt-container">
        ...
        @if(!empty($shop_address) || !empty($shop_city) || !empty($shop_phone) || !empty($shop_email))
        <div class="center meta" style="font-size: 10px; line-height: 1.4;">
            @if(!empty($shop_address)){{ $shop_address }}<br>@endif
            @if(!empty($shop_city) || !empty($shop_postal_code))
                {{ trim(($shop_postal_code ?? '') . ' ' . ($shop_city ?? '')) }}@if(!empty($shop_country)) {{ $shop_country }}@endif<br>
            @endif
            @if(!empty($shop_phone))Tél: {{ $shop_phone }}<br>@endif
            @if(!empty($shop_email)){{ $shop_email }}@endif
        </div>
        @endif
        <div class="center meta">Reçu #{{ $receipt_number ?? $sale['id'] }} · {{ $sale['created_at'] }}</div>
        @if($customer)
    ...
    <script>
        // Si ouvert depuis une autre fenêtre (auto-print après validation), lancer l'impression automatiquement
        if (window.opener) {
